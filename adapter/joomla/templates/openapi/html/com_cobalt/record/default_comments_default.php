...
<div style="margin-left: <?php echo $params->get('tmpl_params.comments_indent') * ($this->comment->level - 1)?>px;" id="comment<?php echo $this->comment->id?>-container" class="">
	<div class="row-fluid">
        ...
        <?php else:?>
            <div class="span12 has-context<?php echo  $this->comment->private ? ' private' : null; echo  !$this->comment->published ? ' published' : null?>">
        <?php endif;?>
			...
			<div>
				<?php echo strip_tags($this->comment->comment); ?>
			</div>
			...
			<?php if(!empty($this->comment->attachment)):?>
				...
				<ul>
					<?php foreach ($this->comment->attachment as $attach):?>
						<li>
							<a href="<?php echo $attach->url?>"><?php echo htmlspecialchars($attach->realname);?></a>
