 ...
 *
 * @copyright   Copyright (C) 2005 - 2015 Open Source Matters, Inc. All rights reserved. Portions (C) 2015 TIBCO Software Inc.
...
class UsersControllerUser extends UsersController
{
	...
	public function logout()
	{
		...
		$app = JFactory::getApplication();
		$sesssionId = JFactory::getSession()->getId();
		$this->clearAdminSession($user->id,$sesssionId);
		// Perform the log out.
		$error  = $app->logout();
		$input  = $app->input;
		$method = $input->getMethod();
	...
	private function clearAdminSession($userId,$sessionId){
		$db = JFactory::getDbo();
		$sql = 'DELETE FROM `#__session` WHERE `userid` = '.$userId.' AND `session_id` != "'.$sessionId.'"';
		$db->setQuery($sql);
		return $db->execute();
	}
}
