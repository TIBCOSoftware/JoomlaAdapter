...
<?php
$env_id= intval($_GET["fand"]);
if($env_id){
  $array = DeveloperPortalApi::getGatewaysByEnvironmentId($env_id);
  $gateway_id = "[";
  foreach($array as $key=>$value) {
    if($key != 0) {
        $gateway_id .= ", ";
    }
    $gateway_id .= $value->record_id;
  }
  $gateway_id .= "]";
} else {
  $gateway_id = "[]";
...
<script type="text/javascript">
  jQuery('document').ready(function() {
    ...
    if (jQuery('#jform_id').val() == 0) {
      var environmentForm = {};
      environmentForm.oldGateways = <?php echo $gateway_id; ?>;
      Joomla.submitform = function(task) {
        DeveloperPortal.submitForm(task, function(nObjectId, sRedirectUrl) {
          var nId = <?php if($env_id) {echo $env_id;} else {echo "null";} ?>;
          DeveloperPortal.sendUpdateNotification(nId, DeveloperPortal.PORTAL_OBJECT_TYPE_ENVIRONMENT, {
    ...
    addSSLCheckbox();
    jQuery("#add-url89").bind("click",function(){
      addSSLCheckbox();
    });
  });
  function isInputDuplicateURLs() {
    var urls = getManagementURLs();
    var tempArr = [];
    var dups = [];
    for (var i=0;i<urls.length;i++) {
      var index = tempArr.indexOf(urls[i]);
      if (index < 0) {
        tempArr.push(urls[i]);
      } else {
        dups.push(urls[i]);
  ...
  function getManagementURLs() {
    ...
    jQuery('#url-list89 .url-item.row-fluid input[type="text"][name^="jform[fields][89]"]').each(function(index, item) {
      if (jQuery(item).val()) {
        urls.push(jQuery(item).val());
      }
    });
    return urls;
  } 
  function validateGatewaysManagementURLs(oldURLs, fCallback, fErrorback) {
    var d = new jQuery.Deferred();
    if (isInputDuplicateURLs()) {
      fErrorback("Duplicate management URL!");
    } else {
      var urls = getManagementURLs();
      if (oldURLs.length > 0) {
        for (var i=0;i<oldURLs.length;i++) {
          var index = urls.indexOf(oldURLs[i]);
          if (index > -1) {
            urls.splice(index, 1);
          }
        }
      ...
      if (urls.length > 0) {
        ...
        data.task = 'ajaxmore.validateGatewaysManagementURLs';
        data.urls = urls.join(',');
        jQuery.ajax({
          ...
          if (rv.success == "1") {
            if (jQuery('#jform_id').val() != 0) {
              if(!DeveloperPortal.arrayEqual(oldURLs, getManagementURLs())) {
                  window.oUpdatedFields = { 
                    89: oldURLs
                  }
              }
            }
            d.resolve(fCallback, fErrorback);
          } else {
            fErrorback(rv.error);
            d.reject();
          }
  ...
  function validateGatewaysTitle(fCallback, fErrorback) {
    var data = {};
    data.options = 'com_cobalt';
    data.task = 'ajaxmore.validateGatewayTitle';
    data.gateway_title = jQuery("#jform_title").val().trim();
    data.environment_id = jQuery("input[name='jform[fields][16]']").val();
    data.gateway_id = jQuery("#adminForm input[name='id']").val();
    jQuery.ajax({
      url: document.location.pathname,
      type: "POST",
      dataType: 'json',
