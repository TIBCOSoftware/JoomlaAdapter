...
abstract class JHtmlBehavior
{
	...
	private static function keepfront(){
            $config = JFactory::getConfig();
            $lifetime = ($config->get('lifetime') * 60000);
            $expireTime = 2 * 60000;
            $user = JFactory::getUser();
            $url =  JURI::root().'templates/openapi/js/session_expiration_warn.js';
            $document = JFactory::getDocument();
            $infoContent = file_get_contents($url);
            $info = str_replace('{session_expiration_warn_info}',JText::_("SESSION_EXPIRATION_WARN"), $infoContent);

            if($user->get('guest') == 0 && $lifetime > 3*60000 ){
                $script = 'window.setTimeout('.$info.',';
                $script .=  $lifetime-$expireTime . ');';
            }
